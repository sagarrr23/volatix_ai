...
def breakout_sniper_signal(df: pd.DataFrame, higher_tf_bullish: bool = True) -> dict:
    ...
    # === BUY Logic ===
    if (
        breakout_bar["close"] > recent_high and
        breakout_bar["bb_width"] < bb_threshold and
        volume_spike and
        higher_tf_bullish
    ):
        return {"signal": "BUY", "reason": "Breakout above resistance + volume surge + BB squeeze"}

    # === SELL Logic ===
    if (
        breakout_bar["close"] < recent_low and
        breakout_bar["bb_width"] < bb_threshold and
        volume_spike and
        not higher_tf_bullish
    ):
        return {"signal": "SELL", "reason": "Breakdown below support + volume spike + BB squeeze"}
